...
class ModelOutputs():
    ...
    def __init__(self, model, target_layers):
        self.model = model
        self.feature_extractor = FeatureExtractor(model.features,target_layers)#nn.Sequential(*list(model.children())[:-1]),
        # target_layers)
...
# model.load_state_dict(torch.load(r"D:\peter\FinalShapes\Models\VGG16\0.pt"))
# model.load_state_dict(torch.load(r"D:\peter\FinalShapes\Models\VGG16\1.pt"))
...
save_loc = os.path.join(config.cam_dir,"VGG")
if not os.path.isdir(save_loc):
    os.mkdir(save_loc)
