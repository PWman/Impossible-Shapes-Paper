...
def get_cam(camnet, torch_img):
    ...
    # pred = net_out.tolist().index(max(net_out.tolist())
    # print(net_out.index(max(net_out)))
...
def get_avg_cam(net_name,pretrain=False):
    ...
    for idx, img in enumerate(all_imgs):
        ...
        if ncorrect>0:
            ...
            # plt.savefig(os.path.join(sdir,os.path.basename(img).replace(".bmp",".png")))
            # plt.clf()
    return
...
if __name__ == "__main__":
    ...
    get_avg_cam("VGG16",pretrain=True)
